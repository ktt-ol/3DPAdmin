...
function get_history($mysqli){
    $query ="SELECT * FROM `history` ORDER BY `history`.`PID` DESC";
    if ($mysqli) { 
        $result = mysqli_query($mysqli, $query);
    ...
    foreach ($lasthistory as $row)  {
      ...
      if($row['is_creditprint'] == 1){$creditstyle = "style=\"background-color: #ddffdd\"";}
        ...
        <td>'.$row['operator'].'</td><td>'.$row['description'].'</td><td>'.badgegroup(get_member_group($mysqli, $row['pricecat'])).'</td>
